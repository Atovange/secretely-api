...
async def create_friend_request(user_id: int, token: str = Depends(oauth2_scheme), session: Session = Depends(get_session)):
...
async def accept_friend_request(user_id: int, token: str = Depends(oauth2_scheme), session: Session = Depends(get_session)):
    token_data = decode_access_token(token)
    return crud.accept_friend_request(session=session, sender_id=user_id, reciever_id=token_data.user_id)

@router.get("/friends", tags=["Users"], response_model=list[User], status_code=status.HTTP_202_ACCEPTED)
async def get_friends(token: str = Depends(oauth2_scheme), session: Session = Depends(get_session)):
    token_data = decode_access_token(token)
